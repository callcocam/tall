...
<div class="@if ($disabled) opacity-60 @endif">
    @if ($label || $cornerHint)
        <div class="flex {{ !$label && $cornerHint ? 'justify-end' : 'justify-between' }} mb-1">
            @if ($label)
                <x-dynamic-component :component="Ui::component('label')" :label="$label" :has-error="$hasError" :for="$id" />
            @endif

            @if ($cornerHint)
                <x-dynamic-component :component="Ui::component('label')" :label="$cornerHint" :has-error="$hasError" :for="$id" />
            @endif
        </div>
    @endif
    ...
    <div class="relative rounded-md @unless($shadowless) shadow-sm @endunless">
        @if ($prefix || $icon)
            <div
                ...
                @if ($icon)
                    <x-dynamic-component :component="Ui::component('icon')" :name="$icon" class="w-5 h-5" />
                @elseif($prefix)
        ...
        @if ($suffix || $rightIcon || ($hasError && !$append))
            <div
                ...
                {{ $hasError ? 'text-negative-500' : 'text-secondary-400' }}">
                @if ($rightIcon)
                    <x-dynamic-component :component="Ui::component('icon')" :name="$rightIcon" class="w-5 h-5" />
                @elseif ($suffix)
                    <span class="flex items-center justify-center pr-1">
                        {{ $suffix }}
                    </span>
                @elseif ($hasError)
                    <x-dynamic-component :component="Ui::component('icon')" name="exclamation-circle" class="w-5 h-5" />
                @endif
            </div>
        @elseif ($append)
            {{ $append }}
    ...
    @if ($name && !$errorless)
        <x-dynamic-component :component="Ui::component('error')" :name="$name" />
    @endif
</div>
