    ...
    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
        <div class="mt-1 flex rounded-md shadow-sm" x-data="{
            ...
            <div class="relative flex flex-grow items-stretch focus-within:z-10">
                <input class="block w-full  rounded-none rounded-l-md border-gray-300 pl-2 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    type="{{ $type }}" id="{{ $id }}" readonly  value="{{ data_get($data, sprintf('data.%s', $modelName)) }}" 
                        ...
                        @if ($options)
                            @foreach ($options as $optLabel => $optValue)
                                <li {{-- :class="{ 'bg-indigo-600 text-white': list.id == {{ data_get($data, $field) }} }" --}}
                                    ...
                                    <label for="{{$name }}.{{$optValue}}" class=" py-2 cursor-pointer w-full flex">
                                    ...
                                    <input class="hidden" value="{{$optValue}}"  wire:model="{{ $key }}" type="radio" name="{{ $name }}" id="{{$name }}.{{$optValue}}">
                                </label>
                                </li>
                            @endforeach
                        @endif
