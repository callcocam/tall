...
@if ($confirm)
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-red-500"> A conta <b class="text-xl">{{ $model->name }}</b>
                será excluída em {{ $confirm }}, todos os recursos e dados
                serão excluídos.
            </h3>
            <p wire:poll.1000ms="redirectList" class="mt-1 max-w-2xl text-sm text-gray-800 font-bold">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div class="border-t border-gray-200">
            <div class="bg-white px-4 py-5">
                ...
                @if ($relations = $this->relations)
                    @foreach ($relations as $relation)
                        @if ($result = data_get($model, $relation))
                            <div class="bg-gray-50 px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-6">
                ...
                @endif
                <dt class="text-sm font-medium text-gray-500">
                    Tem certeza de que deseja excluir a conta <b class="text-xl">{{ $model->name }}</b> ?
                    @if (!$security)
                        <br> Digite sua
                        senha
                    ...
                    <ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
                        <li class="pl-3 pr-4 py-3 flex items-center justify-between ">
                            <div class="  flex items-center space-x-3 ">
                                @if (!$security)
                                    <x-input right-icon="eye" placeholder="*****" wire:model="password"
                                        type="password" />
                                    @if ($valid)
                                        <x-toggle value="1" lg
                            ...
                            <div class="ml-4 flex items-center space-x-2">
                                <x-button wire:click="remove" label="{{ __('Excluir cadastro') }}" icon="trash"
                                    red squared />
                                <x-button icon="arrow-left" squared indigo href="{{ route($this->list) }}"
